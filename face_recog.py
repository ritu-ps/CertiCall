...
# Simple face recognition simulation (no complex dependencies)
FACE_RECOG_AVAILABLE = False  # We'll simulate this for now
...
def capture_unknown_face_interface():
    ...
    with col1:
        if st.session_state.unknown_face_image is not None:
            if isinstance(st.session_state.unknown_face_image, np.ndarray):
...
def perform_attendance_check():
    """Perform the basic attendance check"""
    emp = st.session_state.employee_info
    ...
    try:
        ...
        while not stop_button and (time.time() - start_time) < analysis_duration:
            ...
            # Simple face detection using OpenCV (no complex face_recognition library)
    ...
    complete_attendance_process(emp, emp['name'], "Unknown")
    ...
    with col2:
        if st.button("🚨 Register Unknown Face", type="secondary"):
            ...
            if CV2_AVAILABLE:
                ...
                ret, frame = cap.read()
                if ret:
                    st.session_state.unknown_face_image = frame
                cap.release()
            st.rerun()
    ...
    class VideoProcessor:
        ...
        def recv(self, frame):
            ...
            if CV2_AVAILABLE:
                ...
                for (x, y, w, h) in faces:
                    # Simple "unknown face" simulation - randomly detect unknown faces
                    if np.random.random() < 0.05:  # 5% chance per frame
                        ...
                        cv2.putText(img, "Known Face", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                ...
                if unknown_face_detected and (time.time() - self.last_unknown_detection) > 10:
                    ...
                    if self.unknown_face_counter >= 2 and not st.session_state.unknown_face_detected:
                        st.session_state.unknown_face_detected = True
                        # Store the frame for registration
                        st.session_state.unknown_face_image = img.copy()
                
                # Simulate lie detection randomly
                if np.random.random() < 0.02:  # 2% chance per frame
                    timestamp = datetime.now().strftime("%H:%M:%S")
                    behaviors = ["Unusual eye movement", "Voice stress", "Inconsistent head movement"]
